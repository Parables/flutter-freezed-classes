...
cli.commands(
  {
    program: 'ffc', //'flutter-freezed-classes',
    ...
    generate: command({
      ...
      options: {
        schema: option.string
          ...
          .validate(
            `Expected a GraphQL endpoint URL or valid GraphQL schema file with ${code('.graphql')} ${code(
              '.json'
            )} extension`,
            value =>
              value == undefined || validator.isURL(value) || value.endsWith('.graphql') || value.endsWith('.json')
          ),
        ...
        fileName: option.string
          ...
          .default('app_models')
          .description(
            `the name of the output file without the ${code(
              '.dart'
            )} extension. Appends/replaces the fileName in output option`
          ),
      ...
      const schema: string | undefined = setSchema(_schema);

      const _defaultFileName: string = _fileName.endsWith('.dart') ? _fileName.replace('.dart', '') : _fileName;
...
function setSchema(schema: string | undefined): string | undefined {
  if (schema) {
    return schema;
  }
  ...
  if (fs.existsSync('./schema.json')) {
    ...
    console.error(
      ...
      )} Expected a GraphQL schema at the root of project folder or specify the path/URL with the ${chalk.italic.grey(
        '-s'
      )} or ${chalk.italic.grey('--schema')} option`
    );
    process.exit();
  }
}
